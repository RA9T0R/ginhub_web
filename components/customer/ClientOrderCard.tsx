...
import { Star, Clock, ChefHat, Package, Bike, CheckCircle2, XCircle, Receipt } from 'lucide-react'; // ✅ Import Icon ที่นี่
import ReviewModal from '@/components/customer/ReviewModal';

export default function ClientOrderCard({ order }: { order: any }) {
            ...
            <div className="bg-BG_light dark:bg-Dark_BG_light rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-zinc-800 flex flex-col gap-4">
                <div className="flex justify-between items-start border-b border-gray-100 dark:border-zinc-800 pb-3">
                    <div>
                        ...
                        <p className="text-xs text-subtext dark:text-Dark_subtext mt-1">#{order.id.slice(-8).toUpperCase()}</p>
                    </div>
                    {/* ✅ ใช้ StatusUI และ StatusIcon ที่คำนวณได้จากในนี้ */}
                ...
                {/* ... (โค้ดส่วนแสดงรายการอาหารเหมือนเดิม) ... */}
                <div className="space-y-3 mt-2">
                    {order.items.map((item: any) => (
                        <div key={item.id} className="flex justify-between items-center text-sm">
                            ...
                            <span className="font-bold text-Text dark:text-Dark_Text">฿{item.price * item.quantity}</span>
                        </div>
                    ))}
                </div>
                ...
                <div className="flex justify-between items-center pt-3 border-t border-gray-100 dark:border-zinc-800">
                    ...
                    <span className="text-xl font-black text-primary dark:text-Dark_primary">฿{order.totalAmount + order.deliveryFee}</span>
                </div>

                {order.status === 'DELIVERED' && !order.orderRating && (
                    <button
                        onClick={() => setIsReviewOpen(true)}
                        className="w-full mt-2 bg-gray-900 dark:bg-white text-white dark:text-black font-bold py-3 rounded-xl hover:scale-[1.02] transition-transform"
                    >
                        ได้รับอาหารแล้ว / ให้คะแนนรีวิว ⭐️
                    </button>
                )}
